...
def load_and_fill_biases(src_model, src_weights, dst_model, dst_weights):
    ...
    for i, layer in enumerate(model.layer):
        ...
        if layer.type == 'Convolution' or layer.type=='Deconvolution': # or layer.type == 'Scale':
...
def main(args):
    ...
    model_deploy = args.model+'/deploy.prototxt'
    models = os.listdir(args.model)
    model_weight = args.model+'/'
    for model in models:
        if model.find('final.caffemodel') == -1:
            continue
        model_weight = model_weight + model
        print(model_weight)

    output_deploy = args.model+'/PVANET_merge_bn_deploy.prototxt'
    output_weight = args.model+'/merge_bn_weight.caffemodel'


    net = load_and_fill_biases(model_deploy, model_weight, args.model + '.temp.pt', None)

    net = merge_batchnorms_in_net(net)

    process_model(net, args.model + '.temp.pt', output_deploy,
                  [pick_empty_layers, set_engine_caffe],
                  [remove_empty_layers])

    # Store params
    net.save(output_weight)
